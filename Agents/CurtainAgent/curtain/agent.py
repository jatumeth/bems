...
def CurtainAgent(config_path, **kwargs):
    ...
    db_table_Curtain = settings.DATABASES['default']['TABLE_Curtain']
    ...
    #4.1 initialize Curtain device object
    Curtain = apiLib.API(model=model, device_type=device_type, api=api, address=address, macaddress=macaddress, 
    ...
    print("{0}agent is initialized for {1} using API={2} at {3}".format(agent_id,
                                                                        Curtain.get_variable('model'),
                                                                        Curtain.get_variable('api'),
                                                                        Curtain.get_variable('address')))

    # connection_renew_interval = Curtain.variables['connection_renew_interval']
    ...
    class Agent(PublishMixin, BaseAgent):
        ...
        def setup(self):
            ...
            if api == 'classAPI_WeMo':
                ...
                try:
                    Curtain.startListeningEvents(threadingLock,self.updateStatus)
                    self.subscriptionTime=datetime.datetime.now()
        ...
        def deviceMonitorBehavior(self):
            ...
            try:
                ...
                # _data = Curtain.variables
                # message = json.dumps(_data)
                # CurtainMQTT = importlib.import_module("DeviceAPI.classAPI.device.samples." + "iothub_client_sample")
                # CurtainMQTT.iothub_client_sample_run(message)
            except Exception as er:
                print er
        ...
        def postgresAPI(self):
            ...
            try:
                self.cur.execute("SELECT * from relaysw WHERE Curtain_id=%s", (agent_id,))
                if bool(self.cur.rowcount):
                    pass
                else:
                    self.cur.execute(
                        """INSERT INTO relaysw (Curtain_id, last_scanned_time) VALUES (%s, %s);""",
            ...
            try:
                self.cur.execute("""
                    ...
                    WHERE Curtain_id=%s
                 """, (Curtain.variables['status'], datetime.datetime.now(), agent_id))
                self.con.commit()
        ...
        def device_offline_detection(self):
            self.cur.execute("SELECT nickname FROM " + db_table_Curtain + " WHERE Curtain_id=%s",
                             (agent_id,))
            print agent_id
            ...
            self.cur.execute("SELECT network_status FROM " + db_table_Curtain + " WHERE Curtain_id=%s",
                             (agent_id,))
        ...
        def deviceControlBehavior(self,topic,headers,message,match):
            ...
            if self.isPostmsgValid(message[0]):  # check if the data is valid
                setDeviceStatusResult = Curtain.setDeviceStatus(json.loads(message[0]))
        ...
        def deviceIdentifyBehavior(self,topic,headers,message,match):
            ...
            print "Message: {message}\n".format(message=message)
            #step1: change device status according to the receive message
            identifyDeviceResult = Curtain.identifyDevice()
    ...
    Agent.__name__ = 'CurtainAgent'
    return Agent(**kwargs)

def main(argv=sys.argv):
    '''Main method called by the eggsecutable.'''
    utils.default_main(CurtainAgent,
                       description='Curtain agent',
                       argv=argv)
