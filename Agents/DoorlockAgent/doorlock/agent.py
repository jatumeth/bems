...
def DoorlockAgent(config_path, **kwargs):
    ...
    #4.1 initialize Doorlock device object
    Doorlock = apiLib.API(model=model, device_type=device_type, api=api, address=address, macaddress=macaddress, 
    ...
    print("{0}agent is initialized for {1} using API={2} at {3}".format(agent_id,
                                                                        Doorlock.get_variable('model'),
                                                                        Doorlock.get_variable('api'),
                                                                        Doorlock.get_variable('address')))

    # connection_renew_interval = Doorlock.variables['connection_renew_interval']
    ...
    class Agent(PublishMixin, BaseAgent):
        ...
        def setup(self):
            ...
            if api == 'classAPI_WeMo':
                ...
                try:
                    Doorlock.startListeningEvents(threadingLock,self.updateStatus)
                    self.subscriptionTime=datetime.datetime.now()
        ...
        def deviceMonitorBehavior(self):
            ...
            try:
                Doorlock.getDeviceStatus()
                # _data = Doorlock.variables
                # message = json.dumps(_data)
                # DoorlockMQTT = importlib.import_module("DeviceAPI.classAPI.device.samples." + "iothub_client_sample")
                # DoorlockMQTT.iothub_client_sample_run(message)
            except Exception as er:
                print er
        ...
        def postgresAPI(self):
            ...
            try:
                ...
                if bool(self.cur.rowcount):
                    ...
                    self.cur.execute(
                        """INSERT INTO relaysw (Doorlock_id, last_scanned_time) VALUES (%s, %s);""",
            ...
            try:
                self.cur.execute("""
                    ...
                    WHERE Doorlock_id=%s
                 """, (Doorlock.variables['status'], datetime.datetime.now(), agent_id))
                self.con.commit()
        ...
        def device_offline_detection(self):
            self.cur.execute("SELECT nickname FROM " + db_table_Doorlock + " WHERE Doorlock_id=%s",
                             (agent_id,))
            print agent_id
            ...
            self.cur.execute("SELECT network_status FROM " + db_table_Doorlock + " WHERE Doorlock_id=%s",
                             (agent_id,))
        ...
        def deviceControlBehavior(self,topic,headers,message,match):
            ...
            if self.isPostmsgValid(message[0]):  # check if the data is valid
                setDeviceStatusResult = Doorlock.setDeviceStatus(json.loads(message[0]))
        ...
        def deviceIdentifyBehavior(self,topic,headers,message,match):
            ...
            print "Message: {message}\n".format(message=message)
            #step1: change device status according to the receive message
            identifyDeviceResult = Doorlock.identifyDevice()
...
def main(argv=sys.argv):
    ...
    utils.default_main(DoorlockAgent,
                       description='Doorlock agent',
                       argv=argv)
