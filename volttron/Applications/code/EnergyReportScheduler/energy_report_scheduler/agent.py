...
def scheduleragent(config_path, **kwargs):
    ...
    app_name = "energy_report_scheduler"
    ...
    class Agent(PublishMixin, BaseAgent):
        ...
        def __init__(self, **kwargs):
            ...
            try:
                ...
                if self.cur.rowcount != 0:
                    ...
                    self.current_schedule_object = _new_schedule_object['energy_scheduler']
                    print(self.current_schedule_object)
        ...
        def updateScheduleBehavior(self, topic, headers, message, match):
            ...
            try:
                ...
                print("data is {}".format(_data))
                # _launch_file = _data.get('')
                _launch_file = _data.get('path')
                print("launch file is {}".format(_launch_file))
        ...
        def schedule_action(self):
            ...
            if self.flag_time_to_change_status is True:
                if debug_agent: print "{} for Agent:Load {} >> is reporting energy summary"\
                    .format(app_name, agent_id)
                try:
                    params = {"gridimportenergy": 0, "gridimportbill": 0, "grid_avg_power": 0,
                              "solarenergy": 0,
                              "solarbill": 0, "solar_avg_power": 0, "loadenergy": 0,
                              "loadbill": 0, "load_avg_power": 0}
                    ...
                    sendText = "Dear Sisaengtham School,\n " \
                               ...
                               "\n" \
                               "\t1. Grid: Energy Imported={gridimportenergy} kWh, Electricity Bill={gridimportbill} Baht, Average Power={grid_avg_power} W\n" \
                               "\t2. Solar: Energy Generated={solarenergy} kWh, Bill Saving from Solar={solarbill} Baht, Average Power={solar_avg_power} W\n" \
                               "\t3. Load: Energy Consumed={loadenergy} kWh, Actual Bill if Without Solar={loadbill} Baht, Average Power={load_avg_power} W\n" \
                               "\n" \
                               "Best Regard,\n" \
                               ...
                               "Line: @peahive" \
                        .format(date=datetime.datetime.now().date(), gridimportenergy=params['gridimportenergy'],
                                gridimportbill=params['gridimportbill'],
                                grid_avg_power=params['grid_avg_power'],
                                solarenergy=params['solarenergy'],
                                solarbill=params['solarbill'],
                                solar_avg_power=params['solar_avg_power'],
                                loadenergy=params['loadenergy'],
                                loadbill=params['loadbill'],
                                load_avg_power=params['load_avg_power'], )
                    print(sendText)
                    email = EmailService()
                    ...
                    print "{} for Agent: {} >> ERROR reporting energy {}"\
                        .format(app_name, agent_id, er)
    ...
    Agent.__name__ = 'AC Scheduler Agent'
    return Agent(**kwargs)

def main(argv=sys.argv):
    '''Main method called by the eggsecutable.'''
    utils.default_main(scheduleragent,
                       description='AC Scheduler Agent',
                       argv=argv)
